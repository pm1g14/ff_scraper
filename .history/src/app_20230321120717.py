...
def scrape():
    ...
    for row in table.find_elements(By.TAG_NAME, "tr"):
        ...
        try:
            ...
            if "%" in actual:
                filteredActual = actual.replace('%', '')
            if "%" in forecast:
                filteredForecast = forecast.replace('%', '')
            if "%" in previous:
                filteredPrevious = previous.replace('%', '')
            rowParams = RowParameters(date = date, time = time, currency = currency, event = event, actual = filteredActual))
